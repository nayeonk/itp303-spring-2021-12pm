...
if ( !isset($_POST['track_name']) || 
	...
	// Handle the optional fields such as albunm, composer and bytes. Set these to NULL
	...
	// To check that this new song was actually added to the db, have to check the db. Also, you can check the $mysqli->affected_rows property. It returns how many records were affected

	echo "Inserted: " . $mysqli->affected_rows;

	// We can use affected_rows to show a success message that an insert has happened succesfully
	if($mysqli->affected_rows == 1) {
		$isInserted = true;
	}
	else if($mysqli->affected_rows > 1) {
		// SOmehow created more than one song - that's bad
	}
